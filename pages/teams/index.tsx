            ...
            {navigationItem.subNavigation &&
              navigationItem.link.href === navigationItem.link.href && (
                <div className="flex flex-col md:flex-row md:w-full md:justify-between gap-4 md:gap-0">
                  {Object.entries(navigationItem.subNavigation).map(
                    ([gender, teams]) => {
                      if (teams.length > 0)
                        return (
                          <div key={gender} className="">
                            <div className="text-2xl mb-4 md:text-4xl xl:text-5xl w-min text-yellow-400 bg-origin-padding bg-black shadow-[0px_0px_0px_6px_rgba(0,0,0,1)]">
